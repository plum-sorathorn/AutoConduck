"""Pure budget-to-routing tuning calculations and cost projections."""
...
def _shares(records: list[dict[str, Any]] | None, names: list[str]) -> dict[str, float]:
...
def compute_tuning(
    ...
    if len(entries) > 1:
        ...
        for e, price in zip(entries, prices):
            ...
            limits[_name(e)] = new["spend_guard_max_usd_per_min"] * (
                0.3 + 0.7 * weight
            )
    changes = {
...
def project_spend(tunables: dict[str, Any], pool: list[Any], stats_records=None) -> dict[str, Any]:
    entries = _enabled(pool)
    names = [_name(e) for e in entries]
...
def inputs_dict(inputs: SimpleInputs) -> dict[str, Any]:
...
def infer_monthly_budget(
    ...
    *,
    ...
    return max(0.0, float(math.ceil(monthly)))


def auto_tune_inputs(
    stats_records: list[dict[str, Any]] | None, *, days: int = 30, **fixed: Any
) -> SimpleInputs | None:
