...
class TurnGuard:
    ...
    def classify_turn(self, messages: list[dict[str, Any]]) -> TurnClassificationResult:
        ...
        # Calculate complexity drift (turn depth and file sprawl)
        tool_turn_depth = 0
        file_sprawl = set()
        
        # Count backwards from the end of messages to estimate current streak complexity
        import re
        for m in reversed(messages):
            ...
            if r in ("tool", "function"):
                ...
                # Check for tool_calls in assistant
                tc = m.get("tool_calls")
                    ...
                    for call in tc:
                        if isinstance(call, dict) and isinstance(call.get("function"), dict):
                            ...
                            if isinstance(fn_args, str) and fn_args:
                                paths = re.findall(r'\"(?:path|file|target)\"\s*:\s*\"([^\"]+)\"', fn_args)
                                for p in paths:
                                    file_sprawl.add(p)
                    ...
                    for b in mc:
                        if isinstance(b, dict) and b.get("type") == "tool_use":
                            ...
                            if isinstance(input_val, dict):
                                fn_args = json.dumps(input_val)
                                paths = re.findall(r'\"(?:path|file|target)\"\s*:\s*\"([^\"]+)\"', fn_args)
                                for p in paths:
                                    file_sprawl.add(p)
            elif r == "user":
                # Anthropic mixes tool results into user messages
                mc = m.get("content")
                if isinstance(mc, list):
                    has_tool_result = any(isinstance(b, dict) and b.get("type") == "tool_result" for b in mc)
                    if has_tool_result:
                        tool_turn_depth += 1
                        continue
                # Actual user message breaks the streak
                break
                
        # 3. Complexity Drift: Turn depth > 30 or file sprawl > 10
        if tool_turn_depth > 30:
            return TurnClassificationResult(
                is_tool_loop=True,
                is_stagnant=True,
                stagnation_reason="Complexity Drift: Exceeded 30 consecutive tool turns",
                target_action=TurnAction.ESCALATE_SLM,
                tool_call_streak=tool_call_streak,
                error_streak=error_streak,
                last_tool_name=last_tool_name,
        ...
        if len(file_sprawl) > 10:
            return TurnClassificationResult(
                is_tool_loop=True,
                is_stagnant=True,
                stagnation_reason=f"Complexity Drift: File sprawl detected ({len(file_sprawl)} files)",
                target_action=TurnAction.ESCALATE_SLM,
                tool_call_streak=tool_call_streak,
                error_streak=error_streak,
                last_tool_name=last_tool_name,
