...
**EMA integration (fixes the currently-dead signal):** blend realized token usage into the cost value once enough samples exist:
```python
def _entry_effective_value(model, config):
    entry = _entry(model, config)
    ema = _ema_state.get(model)
    if ema and ema.samples >= config.selection.ema_min_samples:   # default 3
        return entry.price_in * ema.prompt_tokens_ema/1000 + entry.price_out * ema.completion_tokens_ema/1000
    return entry.price_in + entry.price_out   # unchanged flat estimate
```
`scaled_cost(model, config=None, *, use_ema=True)` calls this instead of the flat `price_in+price_out` sum; `α=0.1` EMA update logic in `record_usage` is unchanged.
...
- **Ambiguous tiebreaker**: prompt extended to `"Reply with FAST or SLOW, then a space, then a complexity digit 1-9 (1=trivial, 9=very complex). Example: 'FAST 3'."`; parse `^(FAST|SLOW)\s*(\d)?`. If digit present: `resolved = 0.5*heuristic_complexity + 0.5*(digit/9)` (LLM nudges, never overrides outright). If digit missing/unparseable: `resolved = heuristic_complexity` (deterministic, no failure). Tradeoff: ~2-3 extra output tokens per tiebreak call, no extra round trip.
