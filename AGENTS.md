...
- Honor `request.is_disconnected()` for instant cancellation (used in `server/server_streaming.py` streaming loops).
...
- The FastAPI app in `autoconduck/server/server_streaming.py`, with routes installed by `server/server_routes.py`, serves the LiteLLM-backed endpoint: `/v1/chat/completions` and `/v1/models` (the latter returning exactly the three pseudo-models from `PSEUDO_MODELS` (defined in `server/messages_models.py`, imported by `server/messages_api.py`): `autoconduck`, `autoconduck-budget`, `autoconduck-expensive`), plus AutoConduck-owned `/stats` and `/healthz`, and an Anthropic-compatible `/v1/messages` shim (covered by `tests/test_messages_api.py`). `main.py` remains a compatibility facade.

## Key modules

- `routing/`:
  - `dispatcher.py`: thin sequence of semantic router → evaluator → optional tiebreaker, delegating to `routing/fast_graph.py` when `enable_fast_path_graph` is set (default); keep its own logic minimal.
  - `fast_graph.py`: compiled zero-reflection micro-DAG that runs the fast path in under ~0.1 ms (`FastGraphState`, `FastGraph`, `execute_fast_graph`).
  ...
  - `evaluator.py`: `T_i ∈ [0,1]`, stack-trace boost `+0.25`, escalation boost `+0.30`, hysteresis clamp `≤0.50` after escalation `≥0.80`, active de-escalation to fast below `deescalation_threshold` (0.40), tool-loop fast-path suppression, and ambiguous-zone handling; slow rule is `complexity >= slow_threshold (0.75) or (route == slow_path and confidence >= boundary_high)`.
  - `complexity.py` and `complexity_helpers.py`: single-pass regex complexity scoring over 10 config-weighted factors (length, structural, scope_breadth, code_density, abstraction_level, uncertainty_hedge, cross_domain, task_novelty, imperative_strength, multi_step) and its extraction helpers.
  - `pricing.py`: `select_closest()` closest-cost model matching on ln(1+cost) scaled domain (ties→cheaper, degraded exclusion, optional `band` and `max_scaled_cost` filters, `cheapest_enabled()` deterministic fallback), `target_scaled_cost()` with pseudo-model bias (±0.20) and `value_to_cost_gamma`, EMA realized-cost blend α=0.1 (≥ `ema_min_samples` = 3 samples) via `_entry_effective_value`, failover after trailing error rate >20%, spend guard (`is_over_budget`, `spend_guard_*`), file-read cost ceiling (`is_expensive_model`), `presets/pricing_fallback.json`; `select()`/`select_model_by_tier()` are deprecated thin wrappers.
- `auth/`:
...
- `launcher/`:
  - `launcher.py`, `launcher_procs.py`, `launcher_shims.py`: server refcounting, claims serialization, and process discovery; shims, environment/PATH integration, `real_binary_path`, and install/uninstall helpers.
- `cli/`:
...
- `server/`:
  - `server.py`, `server_routes.py`, `server_streaming.py`: FastAPI server application and route handlers.
  - `messages_api.py`, `messages_models.py`, `messages_sse.py`: Anthropic-compatible message normalization, schemas, and SSE streaming translation.
...
- `agents/`: agent adapters (`opencode.py`, `claude_code.py`, `pi.py`, `aider.py`, `cursor.py`, `continue_dev.py`, `kilocode.py`, `generic_openai.py`).
...
- `tui/`: Textual dashboard and `tui/onboarding/` package providing onboarding/monitoring (Ctrl+C is the single quit chord per `tui/keymap.py`; Textual Ctrl+Q is disabled).

## Repository layout

- `auth/` contains provider authentication, key management, and API discovery.
- `cli/` contains command implementations and agent launch helpers.
- `launcher/` contains process lifecycle management, background server daemons, and shell shims.
- `presets/` contains model presets, fallbacks, and LiteLLM catalog ingestion.
- `routing/` contains the synchronous fast-path stack and pricing algorithms.
- `server/` contains the FastAPI server, endpoint handlers, and Anthropic shim translator.
- `orchestrator/` contains the multi-phase LangGraph pipeline and analyst agent execution.
- `agents/` contains adapters for supported coding agents.
- `tui/` contains Textual terminal UI, dashboard, settings, and interactive onboarding.
