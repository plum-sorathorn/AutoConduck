...
from autoconduck.config import Config, normalize_api_base, orchestrator_litellm_params, qualify_model
...
def test_litellm_params_repair_malformed_base_url():
    cfg = Config(custom_models=[{"id": "broken", "base_url": "ttps://api.example"}])
    assert litellm_params_for("broken", cfg)["api_base"] == "https://api.example/v1"
